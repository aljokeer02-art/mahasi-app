  ...
  async function saveVoucher(e: React.FormEvent) {
    ...
    const { data: entry, error: entErr } = await supabase
      ...
      .insert({
        ...
        description: form.description || "سند قبض",
        status: "مسودة",
        voucher_type: "سند_قبض",
    ...
    await supabase.from("journal_lines").insert([
      { entry_id: entry.id, account_id: toAccountId, contact_id: form.contact_id || null, debit: amount, credit: 0, description: form.description },
      { entry_id: entry.id, account_id: form.from_account, contact_id: form.contact_id || null, debit: 0, credit: amount, description: form.description },
    ]);
    ...
    setForm({ date: new Date().toISOString().slice(0, 10), amount: "", to_cash_bank: "", from_account: "", contact_id: "", method: "نقدي", description: "" });
    setShowForm(false);
    setBusy(false);
    load();
  }
      ...
      <div className="card overflow-hidden">
        <table className="w-full table-base">
          ...
          <tbody>
            ...
            {vouchers.length === 0 && (
              <tr><td colSpan={4} className="text-center py-8 text-forest-800/50">لا توجد سندات قبض بعد.</td></tr>
            )}
          </tbody>
        </table>
      </div>
