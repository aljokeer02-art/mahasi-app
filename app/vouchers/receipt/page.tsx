...
export default function ReceiptVoucherPage() {
  ...
  async function saveVoucher(post: boolean) {
    if (!org) return;
    setError("");
    if (!form.to_cash_bank || !form.from_account || !toAmount || !fromAmount) {
    ...
    await supabase.from("journal_lines").insert([
      {
        entry_id: entryId, account_id: toAccountId, contact_id: form.contact_id || null, debit: toAmount, credit: 0, description: form.description,
      ...
      {
        entry_id: entryId, account_id: form.from_account, contact_id: form.contact_id || null, debit: 0, credit: fromAmount, description: form.description,
    ...
    <AppShell>
      <div className="flex items-center justify-between mb-6 no-print">
        ...
        <div className="flex gap-2">
          ...
          <button className="btn-secondary" onClick={() => exportToExcel("سندات_القبض", "سندات القبض", vouchers.map((v) => ({ "رقم السند": v.entry_number, "التاريخ": v.entry_date, "الطريقة": v.payment_method || "", "البيان": v.description || "" })))}>
            تصدير Excel
          </button>
      ...
      {showForm && (
        <div className="card p-5 mb-6 no-print space-y-4">
          ...
          <div>
            ...
            <select className="input mb-2" value={form.to_cash_bank} onChange={(e) => onToCashBankChange(e.target.value)} required>
              <option value="">اختر...</option>
              {cashBanks.map((cb) => <option key={cb.id} value={cb.id}>{cb.accounts?.code} - {cb.accounts?.name}</option>)}
            </select>
            <SideFields label="طرف الاستلام" side={toSide} which="to" accountLabel={cashBanks.find((c) => c.id === form.to_cash_bank)?.accounts?.name || ""} />
          </div>

          <div>
            <label className="text-sm text-forest-800/70 block mb-1">مصدر المبلغ (الحساب المقابل)</label>
            <select className="input mb-2" value={form.from_account} onChange={(e) => onFromAccountChange(e.target.value)} required>
              <option value="">اختر...</option>
              {accounts.map((a) => <option key={a.id} value={a.id}>{a.code} - {a.name}</option>)}
            </select>
            <SideFields label="طرف المصدر" side={fromSide} which="from" accountLabel={accounts.find((a) => a.id === form.from_account)?.name || ""} />
          </div>

          <div>
            <label className="text-sm text-forest-800/70 block mb-1">البيان</label>
            <input className="input" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="مثال: استلام راتب، أو مصارفة عملة" />
          </div>
